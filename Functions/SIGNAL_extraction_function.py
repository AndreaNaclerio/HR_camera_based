...
def _calculate_peak_hr(ppg_signal, fs):
    ...
    working_data, measures = hp.process(ppg_signal, fs) # here it's correct to insert the sFS of the video since the signal has built starting from the variation between frames
    hp.plotter(working_data, measures)
    HR_toolbox = measures['bpm']
    #return hr_peak, HR_toolbox
    return HR_toolbox
...
'''
questa è la parte di codice che più ci interessa. Quello che facevo, era andare ad applicare direttamente la FFT o l'estrazione dei picchi direttamete sul segnale che 
ci veniva restituito dall'algoritmo. Dal momento in cui alcuni filtraggi erano già stati fatti, allora pensavo che non sevisse aggiungere un post-processing ulteriore.
Invece, guardando l'implementazione su matlab, quello che ho notato è che un ulteriore filtraggio (post processing) del rPPG viene effettuato.

per quanto riguarda la variabile labels, credo faccia riferimento al ground truth (labels -> PPG ground truth).
Viene utlizzata in modo che è possibile andare a comparare le metriche su gli stessi intervalli -> da cui è possibile estrarre anche il SNR
'''

def calculate_metric_per_video(predictions, fs=30, diff_flag=True, use_bandpass=True, hr_method='FFT'): #andrebbe anche aggiunto il labels come parametro di input
    ...
    if hr_method == 'FFT':
        ...
        hr_tool  = _calculate_peak_hr(predictions, fs=fs)
    ...
    return hr_tool #hr_label, hr_pred, SNR
